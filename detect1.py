...
# Email worker thread function corrected
def email_worker():
    while not exit_flag.is_set():
        try:
            ...
            if image_path:
                try:
                    ...
                    # Add message body
                    body = f"A weapon was detected at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}."
...
def trigger_alert(image_path=None):
    ...
    telegram_queue.put("Weapon detected")
...
def run(
    weights=ROOT / "yolov5.pt",  # model path or triton URL
    ...
    # Create alerts directory for weapon detection images
    ...
    try:
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            if label and any(weapon_name in label.lower() for weapon_name in weapon_classes):
                ...
                # Handle weapon detection with cooldown to prevent alert spam
                if weapon_detected:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    alert_path = str(alerts_dir / f"weapon_alert_{timestamp}.jpg")
                    
                    # Save alert image in a separate thread
                    def save_alert_image(img, path):
                        cv2.imwrite(path, img)
                        LOGGER.info(f"Weapon alert image saved to {path}")
                    ...
                    if trigger_alert(alert_path):
                        ...
                        LOGGER.info(f"Weapon alert image saved to {alert_path}")
...
def parse_opt(File):
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "yolov5.pt", help="model path or triton URL")
