...
EMAIL_SUBJECT = "ALERT: No Mask Detected!"
...
class AlertManager:
    ...
    def add_alert(self, img, detection_info, confidence, detected_class):
        ...
        try:
            ...
            # Add to queue
            self.alert_queue.put((img.copy(), detection_info, confidence, detected_class))
            return True
        ...
        while self.is_running:
            try:
                if not self.alert_queue.empty():
                    # Get next alert
                    img, detection_info, confidence, detected_class = self.alert_queue.get()
                    ...
                    if current_time - self.last_alert_time > self.min_interval:
                        ...
                        email_img_path = str(self.email_dir / f"{detected_class}_{timestamp}.jpg")
                        ...
                        telegram_thread = threading.Thread(
                            ...
                            args=(detection_info, confidence, detected_class)
                        )
    ...
    def _send_telegram_alert(self, detection_info, confidence, detected_class):
        """Send Telegram alert"""
        try:
            if detected_class == "NoMask":
                alert_message = f"WARNING: Person without mask detected with confidence {confidence:.2f}"
            else:
                alert_message = f"Person with mask detected with confidence {confidence:.2f}"
    ...
    def _send_email_alert(self, image_path, detection_info):
        """Send email with the detected mask/no-mask image"""
        try:
    ...
    try:
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                # Flag to track if we detected anything
                detected = False
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        detected = True  # We found something!
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            detection_info = f"Detection: {label}\nConfidence: {confidence_str}\nSource: {p.name}"
                            
                            # Queue the alert in the alert manager (non-blocking)
                            im0_annotated = annotator.result()
                            
                            # For NoMask we want to prioritize alerts
                            if names[c] == "NoMask":
                                print("No mask detected!")
                                alert_manager.add_alert(im0_annotated, detection_info, confidence, "NoMask")
                            elif names[c] == "Mask":
                                print("Mask detected")
                                # Either comment this out or leave it in if you want alerts for proper mask usage too
                                alert_manager.add_alert(im0_annotated, detection_info, confidence, "Mask")
...
def parse_opt(File):
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "mask.pt", help="model path or triton URL")
